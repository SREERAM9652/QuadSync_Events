...
function AllRegistrants() {
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm('Are you sure you want to remove ALL registrants? This action cannot be undone.')) return;
    try {
      await deleteAllRegistrants(token);
      setRegistrants([]);
      toast.success('🗑️ All registrants removed successfully!');
    } catch {
      toast.error('❌ Failed to remove all registrants');
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    try {
      const headers = ['Name', 'Email', 'Mobile', 'Message', 'Event Title', 'Registration Date'];
      ...
      exportToCSV(`registrants_export_${new Date().toISOString().split('T')[0]}.csv`, headers, rows);
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-500">
                {searchTerm ? 'Try adjusting your search terms' : 'Registrants will appear here once people start signing up for events'}
              </p>
            </div>
          ) : (
            <motion.div
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                                ...
                                <span className="text-sm">
                                  Event: {registrant.eventId?.title || 'Unknown Event'}
                                </span>
                              </div>
